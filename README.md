...
> **Production contract / 生产契约** — v0.1.0 production data access is intentionally narrow: **macOS 26 with Voice Memos build 1380 only**. Any other environment fails closed rather than guessing at a private data format.
>
> v0.1.0 的生产数据访问刻意限定为 **macOS 26 且 Voice Memos build 1380**；其他环境会安全拒绝，不会猜测私有数据格式。
...
### Signed release package / 已签名发布包

The v0.1.0 release channel is a GitHub-hosted PKG for `com.paaatrick.voice-memos-cli`, signed and notarized by Apple. **When a release PKG is published**, download it from [GitHub Releases](https://github.com/patrick-fu/voice-memos-cli/releases), then install it:

v0.1.0 的发布渠道是托管在 GitHub、面向 `com.paaatrick.voice-memos-cli` 的 PKG，并经签名与 Apple 公证。**发布 PKG 后**，从 [GitHub Releases](https://github.com/patrick-fu/voice-memos-cli/releases) 下载并安装：

```sh
sudo installer -pkg <downloaded-vmemo-package>.pkg -target /
vmemo --version
```

The package installs `vmemo` at `/usr/local/bin/vmemo`. This README does not imply that a specific release artifact already exists. Homebrew is planned as a later distribution option, not a current installation path.

该 PKG 将 `vmemo` 安装到 `/usr/local/bin/vmemo`。本文不表示某个具体发布产物已经存在。Homebrew 是后续分发选项，并非当前安装路径。
...
sudo install -m 755 .build/release/vmemo /usr/local/bin/vmemo
vmemo --version
```

A source-built executable is useful for development but is not a substitute for the signed, notarized release package.

源码构建适用于开发，但不等同于已签名、公证的发布 PKG。
...
- [Repository](https://github.com/patrick-fu/voice-memos-cli) · [Releases](https://github.com/patrick-fu/voice-memos-cli/releases) · [Planning record / 规划记录](https://github.com/patrick-fu/voice-memos-cli/issues/1)
- [Documentation / 文档](docs/agent-contract.md) · [Research / 调研](docs/research)
- [Project Pages / 项目 Pages（发布后）](https://patrick-fu.github.io/voice-memos-cli/)
