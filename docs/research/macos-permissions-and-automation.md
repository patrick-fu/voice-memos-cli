...
| `rename/delete` | **Go（条件满足时）** | 唯一写后端是原生 `AXUIElement`；需 Accessibility、GUI、token、显式确认和逐条 pre/post verification。安全策略和是否允许无人值守尚待 issue 5 产品决策。 |
...
**待实机验证：**锁屏、屏幕锁定、无 console GUI login、SSH、CI、LaunchDaemon 和后台 LaunchAgent 是否能可靠完成目标 UI 操作；Apple 本次引用没有承诺这些模式。当前技术门槛是**已解锁、有人登录、可见 GUI、Voice Memos 前台**；是否把无人值守模式纳入 v0.1 支持矩阵，**待 issue 5 产品决策**。无论该决策为何，`kAXErrorCannotComplete`、目标 app 无响应、等待用户输入、窗口/元素缺失或超时都视为不可安全完成。
...
**待产品决策：**上述 token、确认和串行验证是技术/接口 contract；是否允许满足这些条件后的非交互或无人值守调用，仍待 issue 5 决定。研究不得自行单方面改变该命令可用性政策。
...
| 已解锁前台、锁屏、无 GUI login、Voice Memos 未运行/有 modal/无响应 | 已解锁前台场景必须通过完整 token + fresh pre/post verification；其余技术行为与是否可无人值守均待 issue 5 决定，未满足 preflight 时安全停止，不尝试唤醒/解锁或隐式交互。 |
